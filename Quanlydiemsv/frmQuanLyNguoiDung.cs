...
namespace Quanlydiemsv
{
    public partial class frmQuanLyNguoiDung : Form
    {
        ...
        private void btnThemmoi_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                try
                {
                    ...
                    {
                        int count = DAO.InsertLogin(txtTaikhoan.Text, txtMK.Text, txtHoTen.Text, txtMaSo.Text, cboQuyen.Text);
                        if (count > 0)
                        {
        ...
        private void btnSua_Click(object sender, EventArgs e)
        {
            ...
            {
                int count = DAO.UpdateLogin(txtTaikhoan.Text, txtMK.Text, txtHoTen.Text, txtMaSo.Text, cboQuyen.Text, txtMaSo.Text);
                if (count > 0)
                {
        ...
        private void btnXoa_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show("Bạn có chắc chắn muốn xóa ?", "Thông báo", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                ...
                int count = DAO.DeleteLogin(txtMaSo.Text);
