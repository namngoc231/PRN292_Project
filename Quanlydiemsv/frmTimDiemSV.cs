...
namespace Quanlydiemsv
{
    public partial class frmTimDiemSV : Form
    {
        ...
        private void btnTimKiem_Click_1(object sender, EventArgs e)
        {
            ...
            {
                if (txtMaSV.Text.Equals(kq.MaSV) && cboMonHoc.SelectedValue.Equals(kq.MaMon))
                {
                    dgrDIEMSV.DataSource = DAO.GetDataBySQL("SELECT * FROM [dbo].[tblKET_QUA] WHERE MaSV = '" + txtMaSV.Text + "' and MaMon='" + cboMonHoc.SelectedValue.ToString() + "'");
                    check = true;
                }
            }
            ...
            if (check == false)
            {
                MessageBox.Show("Nhập sai thông tin sinh viên!", "Thông báo!");
            }
        }
