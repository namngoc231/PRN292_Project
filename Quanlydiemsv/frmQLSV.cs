...
namespace Quanlydiemsv
{
    public partial class frmQLSV : Form
    {
        ...
        private void button1_Click_1(object sender, EventArgs e)
        {
            ...
            else
            {
                try
                {
                    ...
                    if (tontai == false)
                    {
                        int count = DAO.InsertSinhVien(txtMaSV.Text, txtHoTen.Text, mskNgaySinh.Text, cboGioiTinh.Text, txtDiaChi.Text, cboMalop.SelectedValue.ToString());
                        if (count > 0)
                        {
        ...
        private void button2_Click_1(object sender, EventArgs e)
        {
            ...
            {
                int count = DAO.UpdateSinhVien(txtMaSV.Text, txtHoTen.Text, mskNgaySinh.Text, cboGioiTinh.Text, txtDiaChi.Text, cboMalop.SelectedValue.ToString(), dgrDSSV.CurrentRow.Cells[0].Value.ToString());
                if (count > 0)
                {
        ...
        private void button3_Click_1(object sender, EventArgs e)
        {
            foreach (KetQua list in listKetQua)
            {
                ...
                {
                    checkdelete = true;
                    MessageBox.Show("Bạn phải xóa Mã sinh viên " + txtMaSV.Text + " từ bảng Kết quả", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    break;
                }
            }
            ...
            if (checkdelete == false)
            {
                ...
                {
                    int count = DAO.DeleteSinhVien(txtMaSV.Text);
